...
```{r}
checkN <- rep(NA, length(allN))
for (i in 1:length(allN)) {
  checkN[i] <- ciM0p(allN[i], mleN, data, 0.95)
  }


allN <- 20:120
# from part a, the untransformed N
N.a <- rep(NA, length(allN))
for (i in 1:length(allN)) {
  N.a[i] <- lnlMt(allN[i], data)
}

# from part b, the log-likelihood N
allN2 <- log(allN)
logN <- rep(NA, length(allN2))
for (i in 1:length(allN2)) {
  logN[i] <- lnlMt2(allN2[i], data)
}

# from part c, the profile log-likelihood N
N.c <- rep(NA, length(allN))
for (i in 1:length(allN)) {
  N.c[i] <- lnlM0p(allN[i], data)
}
N.c
...
par(mar=c(3,3,0,0)+0.2, mgp=c(2,0.8,0)) # setting margins, etc
#plot 1: profile likelihood vs. N
plot(N.c,N.a)
#plot2: profile likelihood vs. log(N)
plot(N.c, logN)
```
...
run.huggins <- function() {
  f0 <- list(formula=~1, share = T)
  ft <- list(formula=~time, share = T)
  ftb<-list(formula=~time)
  fh<-list(formula=~mixture)
  fth<-list(formula=~mixture+time)
  
  m0 <-mark(huggins, model='FullHet', model.parameters = list(p=f0))
  mt <-mark(huggins, model='FullHet', model.parameters = list(p=ft))
  mtb <-mark(huggins, model='FullHet', model.parameters = list(p=ftb, c=f0))
  mh <-mark(huggins, model='FullHet', model.parameters = list(p=fh,c=f0))
  mtb<-mark(huggins, model='FullHet', model.parameters = list(p=ftb, c=f0))
  mth<-mark(huggins,model='FullHet', model.parameters=list(p=fh,c=fh))
  
  return(collect.models)
}

pledger.models<-run.huggins()

print(model.table(pledger.models), digits = 4)
