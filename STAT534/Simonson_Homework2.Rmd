...
The pooled estimate of $\hat{N} = 145$ and its associated standard error is $SE = 1.1547$. The estimate of SE in this method is incorrect, and much lower than the true SE from all observations across all three studies.
...
What is the method-of-moments estimate of $N$?
...
What is the log likelihood function for the parameters $(N,\pi_s)$ given the data, $(Y,Z_1+Z_2)$ and known constant, $\pi_a$. Use the Binomial model for $Y$.
...
Find the mle's of $N$ and $\pi_s$ and their standard errors.
...
Find the mle's of $N$ and $\pi_s$ and their standard errors under the Poisson model for $Y$. That is, $Y \sim Pois(N,\pi_a,\pi_s)$. _Note:_ the searcher trial model remains a Binomial because the number of carcasses is not random.
