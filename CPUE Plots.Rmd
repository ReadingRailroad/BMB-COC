...
write.csv(DF2017, "2017/2017 Raw CPUE Data.csv",row.names = F)
...
colnames(coc.cpue2)<-c("Year","Month","Day","Lake","Run")
coc.cpue2


# buffalo
bib.cpue$DLR<-as.character(bib.cpue$DLR)
bib.cpue2<-data.frame(do.call(rbind,strsplit(bib.cpue$DLR,
                                      "\\-|\\-|\\;|\\;")))
# need to adjust column names then we are in business
colnames(bib.cpue2)<-c("Year","Month","Day","Lake","Run")

# Join it up like Bob Marley
final1<-data.frame(c(coc.cpue2,coc.cpue[,3:5]))
final1$Species<-"COC"
final2<-data.frame(c(bib.cpue2,bib.cpue[,3:5]))
final2$Species<-"BIB"

final<-rbind(final1,final2)

```
- writing .csv at this point. 
going to finish summary stats and plotting in 2019 annual report file

```{r}
write.csv(final, "2018 Raw CPUE Data.csv", row.names = F)
...
### 2019 CPUE data wrangling

objective is to create a dataframe with the following columns:

Date, Lake, Species, CPUE, Transect #


```{r}
# load in data
frame2019<-read.csv("2017-2020 Compiled Data.csv", header = T)
str(frame2019)

# trim the fat

frame2019<-frame2019[,c(1,2,4,10:13,17,21,22)]
frame2019$Year<-as.factor(frame2019$Year)
frame2019<-droplevels(subset(frame2019, Year == "2019"))

# try to set the date column
frame2019$Date <- as.Date(frame2019$Date , "%m/%d/%Y")
str(frame2019)

# sort lake names:
levels(frame2019$Lake)

# subset the species we want, carp buffalo, and dummy
levels(frame2019$Species)
frame2019<-droplevels(subset(frame2019, Species == "dummy" |
                                        Species == "COC" |
                                        Species == "BIB"))
levels(frame2019$Species)

# electrofishing only
levels(frame2019$Gear)
frame2019<-droplevels(subset(frame2019, Gear == "Electroshocking"))

# sort out the standard transects
levels(frame2019$Start.Latitude)

stdRuns<-droplevels(subset(frame2019, Start.Latitude == "5 Island  trans. 3" |
                             Start.Latitude == "5 Island tans. 4"   |
                             Start.Latitude == "5 Island trans 1"   |
                             Start.Latitude == "5 Island trans 2"   |
                             Start.Latitude == "5 Island trans 3"   |
                             Start.Latitude == "5 Island trans 4"   |
                             Start.Latitude == "5 Island trans 5"   |
                             Start.Latitude == "5 Island trans 5 "  |
                             Start.Latitude == "5 Island trans. 1"  |
                             Start.Latitude == "5 Island trans. 2"  |
                             Start.Latitude == "5 island trans. 3"  |
                             ...
                             Start.Latitude == "5 island trans. 5"  |
                             Start.Latitude == "5 Island trans. 5"  |
                             Start.Latitude == "5I tans 2"          |
                             Start.Latitude == "5I trans 1"         |
                             Start.Latitude == "5I trans 3"         |
                             Start.Latitude == "5I trans 4"         |
                             Start.Latitude == "5I trans 5"         |
                             Start.Latitude == "Blue trans. 1"      |
                             Start.Latitude == "Blue trans. 2"      |
                             Start.Latitude == "Blue trans. 3"      |
                             Start.Latitude == "Blue trans. 4"      |
                             ...
                             Start.Latitude == "N Twin Std. 4"      |
                             Start.Latitude == "N. Twin trans. 1"   |
                             Start.Latitude == "N. Twin trans. 2"   |
                             Start.Latitude == "S. Twin trans. 1"   |
                             Start.Latitude == "S. Twin trans. 2"   |
                             Start.Latitude == "S. Twin trans. 3"   |
                             Start.Latitude == "S. Twin trans. 4"   |
                             Start.Latitude == "Siliver trans. 1"   |
                             Start.Latitude == "SIliver trans. 2"   |
                             Start.Latitude == "Silver tans 2"      |
                             ...
                             Start.Latitude == "Silver trans. 6"    |
                             Start.Latitude == "SIlver trans. 6"    |
                             Start.Latitude == "Silverr trans. 6"   |
                             Start.Latitude == "Slver trans. 2"     |
                             Start.Latitude == "Slver trans. 2 "    |
                             Start.Latitude == "STD 1 by fen"       |
                             Start.Latitude == "STD 1 start"        |
                             Start.Latitude == "STD 2"              |
                             Start.Latitude == "Std 2 start"        |
                             Start.Latitude == "STD 2 start"        |
                             Start.Latitude == "Std 3 start"        |
                             Start.Latitude == "Std 4 start"        |
                             Start.Latitude == "Std 5 start"        |
                             Start.Latitude == "Std1 start"         |
                             Start.Latitude == "Storm  trans 5"     |
                             Start.Latitude == "Storm tans 7"       |
                             ...
                             Start.Latitude == "Storm trans. 2"     |
                             Start.Latitude == "Storm trans. 2 "    |
                             Start.Latitude == "Storm trans. 3"     |
                             Start.Latitude == "Storm trans. 4"     |
                             Start.Latitude == "Storm trans. 5"     |
                             Start.Latitude == "Storm trans. 6"     |
                             Start.Latitude == "Storm trans. 7"     |
                             Start.Latitude == "Storm trans. 9"     |
                             Start.Latitude == "Strom trans 1"      |
                             Start.Latitude == "Strom trans 4 "     |
                             Start.Latitude == "Strom trans 5"      |
                             Start.Latitude == "Strom trans. 4"     |
                             Start.Latitude == "Strom trans. 7"     |
                             Start.Latitude == "Strom trans. 9" ))

levels(stdRuns$Site.Transect)
stdRuns<-droplevels(subset(stdRuns, Site.Transect != "Bonus 1" ))
stdRuns<-droplevels(subset(stdRuns, Site.Transect != "Bonus 2" ))
stdRuns<-droplevels(subset(stdRuns, Site.Transect != "Bonus 4" ))

# set transect levels to be consistent among lakes and years
levels(stdRuns$Site.Transect)

# trim out notes and gear and start latitude columns
str(stdRuns)
stdRuns<-stdRuns[,-c(4,5,7)]

# check that the on-time in seconds is approximately 900 seconds
summary(stdRuns$On.Time..s.)
# center lake transect 1 on 5-14-2019
stdRuns<-droplevels(subset(stdRuns, On.Time..s. != 1909))

stdRuns<-droplevels(subset(stdRuns, On.Time..s. != 1493))

# north twin lake on 5-17
stdRuns<-droplevels(subset(stdRuns, On.Time..s. != 1430))
stdRuns<-droplevels(subset(stdRuns, On.Time..s. != 1069))
stdRuns<-droplevels(subset(stdRuns, On.Time..s. != 678))
stdRuns<-droplevels(subset(stdRuns, On.Time..s. != 575))
summary(stdRuns$On.Time..s.)


# adding an effort column, the on-time divided by an hour (3600 seconds)
stdRuns$Effort<-stdRuns$On.Time..s./3600

cpue<-aggregate(detect~Date+Lake+Site.Transect+Species+Effort, 
                data = stdRuns, sum, na.action=NULL)

# for two species and "dummy" for goose eggs
head(cpue)

# let me assign seasons here to get a groupwise mean
cpue$Season[cpue$Date <= "2019-05-31"] <- "Spring"
cpue$Season[cpue$Date >= "2019-06-01" & cpue$Date <= "2019-07-31"] <- "Summer"
cpue$Season[cpue$Date >= "2019-08-01"] <- "Fall"
length(is.na(cpue$Season))

# ignoring storm because it's problematic
cpue<-droplevels(subset(cpue, Lake != "Storm"))
# ignoring blue and south twin for ia afs 2020
cpue<-droplevels(subset(cpue, Lake != "Blue"))
cpue<-droplevels(subset(cpue, Lake != "S Twin"))

# finding out about 'std' runs '6' in five island and '7' in silver
junks<-droplevels(subset(cpue, Lake == "5 Island" & Date == "2019-08-02"))
# these are the std runs, just numbered 3-7 and not 1-5 for some reason :(
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-08-02" &
                   cpue$Site.Transect == "Std 3"]<-"Std 1"
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-08-02" &
                   cpue$Site.Transect == "Std 4"]<-"Std 2"
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-08-02" &
                   cpue$Site.Transect == "Std 5"]<-"Std 3"
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-08-02" &
                   cpue$Site.Transect == "Std 6"]<-"Std 4"
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-08-02" &
                   cpue$Site.Transect == "Std 7"]<-"Std 5"
junks<-droplevels(subset(cpue, Lake == "5 Island" & Date == "2019-08-02"))

# now for five island but on june 25
junks<-droplevels(subset(cpue, Lake == "5 Island" & Date == "2019-06-25"))
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-06-25" &
                   cpue$Site.Transect == "Std 4"]<-"Std 3"
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-06-25" &
                   cpue$Site.Transect == "Std 5"]<-"Std 4"
cpue$Site.Transect[cpue$Lake == "5 Island" & 
                   cpue$Date == "2019-06-25" &
                   cpue$Site.Transect == "Std 6"]<-"Std 5"
junks<-droplevels(subset(cpue, Lake == "5 Island" & Date == "2019-06-25"))

# now for silver on 6-3
junks<-droplevels(subset(cpue, Lake == "Silver" & Date == "2019-06-03"))
cpue$Site.Transect[cpue$Lake == "Silver" & 
                   cpue$Date == "2019-06-03" &
                   cpue$Site.Transect == "Std 7"]<-"Std 6"

# setting dummy to coc and bib
coc.extra<-droplevels(subset(cpue, Species == "dummy"))
...
cpue$detect[cpue$Species == "dummy"] <- 0
cpue$Species[cpue$Species == "dummy"] <- "BIB"

# adding on coc.extra
cpue<-rbind(cpue, coc.extra)

# NOW! big problem in figuring out how to add 0 catch for one species when the other was observed.....
...
cpue$cpue<-cpue$detect/cpue$Effort




# finding out which date/transect combos don't match between species
buff1<-droplevels(subset(cpue, Species == "BIB"))
carp1<-droplevels(subset(cpue, Species == "COC"))

buff1$DLR<-paste(buff1$Date,buff1$Lake,buff1$Site.Transect, sep = "-")
carp1$DLR<-paste(carp1$Date,carp1$Lake,carp1$Site.Transect, sep = "-")


setdiff(buff1$DLR,carp1$DLR) # entries in buff frame not in carp frame
str(cpue)
a <- c("2019-09-22","Center","Std 4","COC",NA,0,"Fall",0)
b <- c("2019-09-22","Center","Std 5","COC",NA,0,"Fall",0)
c <- c("2019-08-06","5 Island","Std 1","COC",NA,0,"Fall",0)
d <- c("2019-08-05","Center","Std 1","COC",NA,0,"Fall",0)  
e <- c("2019-09-07","Center","Std 3","COC",NA,0,"Fall",0)
f <- c("2019-05-27","5 Island","Std 2","COC",NA,0,"Spring",0)
g <- c("2019-09-21","5 Island","Std 2","COC",NA,0,"Fall",0)
h <- c("2019-09-21","5 Island","Std 1","COC",NA,0,"Fall",0)
i <- c("2019-09-22","Center","Std 1","COC",NA,0,"Fall",0)
j <- c("2019-09-07","Center","Std 1","COC",NA,0,"Fall",0)
k <- c("2019-09-07","Center","Std 2","COC",NA,0,"Fall",0)
l <- c("2019-09-22","Center","Std 3","COC",NA,0,"Fall",0)  
m <- c("2019-05-27","5 Island","Std 5","COC",NA,0,"Spring",0)

add.on<-data.frame(t(data.frame(a,b,c,d,e,f,g,h,i,j,k,l,m)),
                   stringsAsFactors = F)

setdiff(carp1$DLR,buff1$DLR) # entries in carp frame not in buff frame
a <- c("2019-06-03","Center","Std 4","BIB",NA,0,"Summer",0)
b <- c("2019-04-27","Center","Std 4","BIB",NA,0,"Spring",0)
c <- c("2019-06-03","Center","Std 3","BIB",NA,0,"Summer",0)
d <- c("2019-04-27","Center","Std 2","BIB",NA,0,"Spring",0)
e <- c("2019-07-30","Center","Std 3","BIB",NA,0,"Summer",0)
f <- c("2019-07-30","Silver","Std 1","BIB",NA,0,"Summer",0)
g <- c("2019-09-08","5 Island","Std 2","BIB",NA,0,"Fall",0)
h <- c("2019-09-08","5 Island","Std 3","BIB",NA,0,"Fall",0)
i <- c("2019-08-06","5 Island","Std 4","BIB",NA,0,"Fall",0)
j <- c("2019-08-05","Center","Std 4","BIB",NA,0,"Fall",0)
k <- c("2019-07-30","Silver","Std 4","BIB",NA,0,"Summer",0)
l <- c("2019-07-30","Silver","Std 6","BIB",NA,0,"Summer",0)  
m <- c("2019-09-21","5 Island","Std 4","BIB",NA,0,"Fall",0)
n <- c("2019-07-30","Silver","Std 2","BIB",NA,0,"Summer",0)
o <- c("2019-08-02","5 Island","Std 1","BIB",NA,0,"Fall",0)
p <- c("2019-09-21","Silver","Std 5","BIB",NA,0,"Fall",0)  
q <- c("2019-06-14","5 Island","Std 5","BIB",NA,0,"Summer",0)
r <- c("2019-08-05","Center","Std 2","BIB",NA,0,"Fall",0)
s <- c("2019-08-02","5 Island","Std 4","BIB",NA,0,"Fall",0)
t <- c("2019-06-03","Silver","Std 2","BIB",NA,0,"Summer",0)  
u <- c("2019-10-20","5 Island","Std 5","BIB",NA,0,"Fall",0)
v <- c("2019-06-18","Silver","Std 5","BIB",NA,0,"Summer",0)
w <- c("2019-08-02","5 Island","Std 3","BIB",NA,0,"Fall",0)
x <- c("2019-09-21","5 Island","Std 5","BIB",NA,0,"Fall",0)
y <- c("2019-07-30","Center","Std 1","BIB",NA,0,"Summer",0)
z <- c("2019-09-21","Silver","Std 1","BIB",NA,0,"Fall",0)

aa<- c("2019-06-18","Silver","Std 2","BIB",NA,0,"Summer",0)
bb<- c("2019-07-30","Silver","Std 3","BIB",NA,0,"Summer",0)  
cc<- c("2019-08-06","5 Island","Std 2","BIB",NA,0,"Fall",0)
dd<- c("2019-08-02","5 Island","Std 5","BIB",NA,0,"Fall",0)
ee<- c("2019-09-21","Silver","Std 3","BIB",NA,0,"Fall",0)
ff<- c("2019-06-11","Center","Std 1","BIB",NA,0,"Summer",0)  
gg<- c("2019-09-21","5 Island","Std 3","BIB",NA,0,"Fall",0)
hh<- c("2019-08-06","5 Island","Std 5","BIB",NA,0,"Fall",0)
ii<- c("2019-06-17","5 Island","Std 2","BIB",NA,0,"Summer",0)
jj<- c("2019-06-18","Silver","Std 1","BIB",NA,0,"Summer",0)  
kk<- c("2019-06-25","5 Island","Std 1","BIB",NA,0,"Summer",0)
ll<- c("2019-08-02","5 Island","Std 2","BIB",NA,0,"Fall",0)
mm<- c("2019-09-21","Silver","Std 6","BIB",NA,0,"Fall",0)
nn<- c("2019-06-17","5 Island","Std 1","BIB",NA,0,"Summer",0)
oo<- c("2019-09-21","Silver","Std 4","BIB",NA,0,"Fall",0)
pp<- c("2019-06-17","5 Island","Std 3","BIB",NA,0,"Summer",0)
qq<- c("2019-06-18","Silver","Std 6","BIB",NA,0,"Summer",0)
rr<- c("2019-09-08","5 Island","Std 5","BIB",NA,0,"Fall",0)
ss<- c("2019-06-18","Silver","Std 3","BIB",NA,0,"Summer",0)
tt<- c("2019-05-29","Center","Std 1","BIB",NA,0,"Spring",0)
uu<- c("2019-06-03","Silver","Std 1","BIB",NA,0,"Summer",0)
vv<- c("2019-05-20","Silver","Std 6","BIB",NA,0,"Spring",0)  
xx<- c("2019-06-25","5 Island","Std 5","BIB",NA,0,"Summer",0)
yy<- c("2019-06-25","5 Island","Std 3","BIB",NA,0,"Summer",0)
zz<- c("2019-05-14","Center","Std 3","BIB",NA,0,"Spring",0)

aaa<-c("2019-06-17","5 Island","Std 4","BIB",NA,0,"Summer",0)
bbb<-c("2019-06-25","5 Island","Std 4","BIB",NA,0,"Summer",0)
ccc<-c("2019-04-27","Center","Std 1","BIB",NA,0,"Spring",0)
ddd<-c("2019-06-14","5 Island","Std 2","BIB",NA,0,"Summer",0)
eee<-c("2019-06-25","5 Island","Std 2","BIB",NA,0,"Summer",0)
fff<-c("2019-05-27","5 Island","Std 4","BIB",NA,0,"Spring",0)
ggg<-c("2019-06-12","Silver","Std 1","BIB",NA,0,"Summer",0)
hhh<-c("2019-06-14","5 Island","Std 1","BIB",NA,0,"Summer",0)

add.on2<-data.frame(t(data.frame(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,
                                 aa,bb,cc,dd,ee,ff,gg,hh,ii,jj,kk,ll,mm,nn,oo,pp,qq,rr,ss,tt,uu,vv,xx,yy,zz,
                                 aaa,bbb,ccc,ddd,eee,fff,ggg,hhh)),
                   stringsAsFactors = F)
names(add.on)<-c("Date","Lake","Site.Transect","Species","Effort","detect","Season","cpue")
names(add.on2)<-c("Date","Lake","Site.Transect","Species","Effort","detect","Season","cpue")


# throw that ish together
cpue2<-rbind(cpue,add.on,add.on2)
cpue2<-droplevels(cpue2)
str(cpue2)
cpue2$Effort<-as.numeric(as.character(cpue2$Effort))
cpue2$detect<-as.numeric(as.character(cpue2$detect))
cpue2$Season<-as.factor(cpue2$Season)
cpue2$cpue<-as.numeric(as.character(cpue2$cpue))
str(cpue2)

cpue3<-aggregate(cpue ~ Lake + Species + Site.Transect + Season, cpue2,mean)

cpue.m<-melt(cpue3)

cpue.cast<-dcast(cpue.m, Lake + Site.Transect + Season ~ Species, value.var = "value")
...
```{r,}
# OK I'm gonna send this to Sarah for now, and see what she thinks and needs it to change.
write.csv(cpue.cast, "Hanna_2019_CPUE.csv", row.names = F)
```
