...
```{r}
# load in data
frame2019<-read.csv("2019/2019 CPUE efishing data.csv", header = T)
str(frame2019)

# trim the fat

frame2019<-frame2019[,c(1,2,4,10:13,17)]
...
frame2019$Lake[frame2019$Lake == "5 Island "] <- "5 Island"
droplevels(frame2019$Lake)
...
frame2019<-droplevels(subset(frame2019, Gear != "Seine "))
...
levels(stdRuns$Transect)
stdRuns$Transect[stdRuns$Transect == "11"] <- "2"
stdRuns<-droplevels(subset(stdRuns, Transect != "Bonus 1" ))
stdRuns<-droplevels(subset(stdRuns, Transect != "Bonus 2" ))
stdRuns<-droplevels(subset(stdRuns, Transect != "Bonus 4" ))

# set transect levels to be consistent among lakes and years
levels(stdRuns$Transect)

stdRuns$Transect[stdRuns$Transect == "Standard 2"] <- "2"
stdRuns$Transect[stdRuns$Transect == "Standard 3"] <- "3"
stdRuns$Transect[stdRuns$Transect == "Standard 4"] <- "4"
stdRuns$Transect[stdRuns$Transect == "Standard 5"] <- "5"
stdRuns$Transect[stdRuns$Transect == "Standard1"] <- "1"
stdRuns$Transect[stdRuns$Transect == "Std 1"] <- "1"
stdRuns$Transect[stdRuns$Transect == "Std 2"] <- "2"
stdRuns$Transect[stdRuns$Transect == "Std 3"] <- "3"
stdRuns$Transect[stdRuns$Transect == "Std 4"] <- "4"
stdRuns$Transect[stdRuns$Transect == "Std 5"] <- "5"
stdRuns$Transect[stdRuns$Transect == "Storm  trans 5"] <- "5"
stdRuns$Transect[stdRuns$Transect == "Storm 5"] <- "5"
stdRuns$Transect[stdRuns$Transect == "Storm 7"] <- "7"
stdRuns$Transect[stdRuns$Transect == "Storm trans 2"] <- "2"
stdRuns$Transect[stdRuns$Transect == "Storm trans 4"] <- "4"
stdRuns$Transect[stdRuns$Transect == "Storm trans 9"] <- "9"
stdRuns$Transect[stdRuns$Transect == "Strom trans 1"] <- "1"
stdRuns$Transect<-droplevels(stdRuns$Transect)
levels(stdRuns$Transect)
...
# adding a dummy variable for observation as "count" of species
stdRuns$Obs<-1

# adding an effort column, the on-time divided by an hour (3600 seconds)
stdRuns$Effort<-stdRuns$On.time..s./3600

cpue<-aggregate(Obs~Date+Lake+Transect+Species+Effort, data = stdRuns, sum, na.action=NULL)
cpue$cpue<-cpue$Obs/cpue$Effort
