...
carp<-ggplot(DF, aes(x=CarpBiomass, y=C.MeanCPUE)) +
  ...
  geom_errorbarh(aes(xmax= CarpBM.UL, xmin = CarpBM.LL),height=5) +
  annotate("text",
           x=c(135000,30000,200000,8000,37000,100000,60000,15000,15000),
           y=c(200,8,30,25,45,25,30,180,160), 
...
stdRuns<-droplevels(subset(stdRuns, Site.Transect != "Bonus 3" ))
stdRuns<-droplevels(subset(stdRuns, Site.Transect != "Bonus 4" ))
stdRuns<-droplevels(subset(stdRuns, Site.Transect != "Bonus 7" ))
...
n <- c("2019-06-18","Silver","Std 6","COC",NA,0,"Summer",0)

add.on<-data.frame(t(data.frame(a,b,c,d,e,f,g,h,i,j,k,l,m,n)),
                   stringsAsFactors = F)
...
jj<- c("2019-06-18","Silver","Std 4","BIB",NA,0,"Summer",0)  
...
tt<- c("2019-06-18","Silver","Std 1","BIB",NA,0,"Spring",0)
...
hhh<-c("2019-06-14","5 Island","Std 4","BIB",NA,0,"Summer",0)

add.on2<-data.frame(t(data.frame(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,
                                 aa,bb,cc,dd,ee,ff,gg,hh,ii,jj,kk,ll,mm,nn,oo,pp,qq,rr,ss,tt,uu,vv,xx,yy,
...
# triple check those zero fish runs
buff1<-droplevels(subset(cpue2, Species == "BIB"))
carp1<-droplevels(subset(cpue2, Species == "COC"))
...
cpue5<-groupwiseMean(cpue~Lake+Species+Season,cpue2)
