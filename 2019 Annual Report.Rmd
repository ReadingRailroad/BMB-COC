...
```{r}
# front matter
DF<-read.csv("2018BiomassCPUEtable.csv",header=T)
str(DF)
DF<-DF[,-1]
```
...
(2) stacked plots with each month as a panel in one column, biomass density on x axis and CPUE in the multiple y-axes. side by side for each species first but also split by STD and Run-n-Gun within each species.  
