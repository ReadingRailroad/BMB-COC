...
CI95.plot<-ggplot(CI95.b, aes(x=Month, y=Mean))+
  geom_jitter(aes(colour = Lake, shape = Lake),
              size=4, width = 0.2)+
  scale_shape_manual(values = c(15,16,17,2,3,4,8))+
  theme_bw()+
  ...
  labs(x="",
       ...
       title = "Electrofishing CPUE by Month")
...
fitTyp<-nls(tl~vbTyp(age,Linf,K,t0), 
            ...
            data=b.carp, start=svTypical) ## ERROR
...
fitPlot(fitTypical,xlab="Age",ylab="Total Length (mm)",
        main = "2017 Center Lake Carp VBGM - Spines Only",
...
nt.carp$age<-nt.carp$Otolith.MAS 
...
#svAlternate<-vbStartsDP(TL.mm,Otolith.MAS,data=b.carp)
unlist(svTypical)
svTypical<-list(Linf=735.296,K=0.1600955,t0=-1.5186606)
...
vbTypical <- tl ~ Linf*(1-exp(-K*(age-t0)))
vbTyp<-vbFuns()
vbTyp(3,Linf=c(765,0.1600955,-1.5186606))
...
ages2plot<- 0:24                        # need to change this with each spp/lake combo
LCI<-UCI<-numeric(length(ages2plot))
for(i in 1:length(ages2plot)) {
  pv <- ests[,"Linf"]*(1-exp(-ests[,"K"]*(ages2plot[i]-ests[,"t0"])))
  LCI[i] <-quantile(pv,0.025)
...
fitPlot(fitTypical,xlab="Age",ylab="Total Length (mm)",
        main = "2017 North Twin Lake Carp VBGF",
        xlim = range(ages2plot), ylim = c(200,900),
        legend = T)
lines(UCI~ages2plot,type="l",col="blue",lwd=2,lty=2)
lines(LCI~ages2plot,type="l",col="blue",lwd=2,lty=2)
...
st.carp$age<-st.carp$Otolith.MAS # otoliths taken from this lake 2017
...
#svAlternate<-vbStartsDP(TL.mm,Otolith.MAS,data=b.carp)
unlist(svTypical)
svTypical<-list(Linf=619,K=0.144879,t0=-8.042712)
...
vbTypical <- tl ~ Linf*(1-exp(-K*(age-t0)))
vbTyp<-vbFuns()
vbTyp(3,Linf=c(619,0.144879,-8.042712))
...
ages2plot<- 0:28                       # need to change this with each spp/lake combo
LCI<-UCI<-numeric(length(ages2plot))
for(i in 1:length(ages2plot)) {
  pv <- ests[,"Linf"]*(1-exp(-ests[,"K"]*(ages2plot[i]-ests[,"t0"])))
  LCI[i] <-quantile(pv,0.025)
...
fitPlot(fitTypical,xlab="Age",ylab="Total Length (mm)",
        main = "2017 South Twin Lake Carp VBGF",
        xlim = range(ages2plot), ylim = c(200,900),
        legend = T)
lines(UCI~ages2plot,type="l",col="blue",lwd=2,lty=2)
lines(LCI~ages2plot,type="l",col="blue",lwd=2,lty=2)
...
nt.buff$age<-nt.buff$Otolith.MAS 
...
#svAlternate<-vbStartsDP(TL.mm,Otolith.MAS,data=b.buff)
unlist(svTypical)
svTypical<-list(Linf=622.79,K=0.3775477,t0=-0.1895637)
...
vbTypical <- tl ~ Linf*(1-exp(-K*(age-t0)))
vbTyp<-vbFuns()
vbTyp(3,Linf=c(622.79,0.3775477,-0.1895637))
...
ages2plot<- 0:22                        # need to change this with each spp/lake combo
LCI<-UCI<-numeric(length(ages2plot))
for(i in 1:length(ages2plot)) {
  pv <- ests[,"Linf"]*(1-exp(-ests[,"K"]*(ages2plot[i]-ests[,"t0"])))
  LCI[i] <-quantile(pv,0.025)
...
fitPlot(fitTypical,xlab="Age",ylab="Total Length (mm)",
        main = "2017 North Twin Lake Buffalo VBGM",
        xlim = range(ages2plot), ylim = c(200,900),
        legend = T)
lines(UCI~ages2plot,type="l",col="blue",lwd=2,lty=2)
lines(LCI~ages2plot,type="l",col="blue",lwd=2,lty=2)
...
st.buff$age<-st.buff$Otolith.DRF # No otoliths taken from this lake 2017
...
#svAlternate<-vbStartsDP(TL.mm,Otolith.MAS,data=b.buff)
unlist(svTypical)
svTypical<-list(Linf=591.78,K=0.1976491,t0=-6.3982946) # screwed up starting values... try spines
...
vbTypical <- tl ~ Linf*(1-exp(-K*(age-t0)))
vbTyp<-vbFuns()
vbTyp(3,Linf=c(591.78,0.1976491,-6.3982946))
...
str(diversity)
psd.frame<-droplevels(subset(diversity, Species == "BLC"|
                                        Species == "BLG"|
                                        Species == "CCF"|
                                        Species == "LMB"|
                                        Species == "WAE"|
...
psd.frame$PSDcat<-psdAdd(Length..mm.~Species, data = psd.frame,
                        units = "mm")
head(psd.frame)
( t1 <- with(psd.frame, table(interaction(Species, Lake),PSDcat)) )
