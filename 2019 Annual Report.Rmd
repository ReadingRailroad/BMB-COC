...
st.buff$age<-st.buff$Otolith.MAS # No otoliths taken from this lake 2017

summary(st.buff$tl)
summary(st.buff$age)

# vbStarts and function variables
svTypical<-vbStarts(tl~age,data=st.buff, meth0 = "poly")
#svAlternate<-vbStartsDP(TL.mm,Otolith.MAS,data=b.buff)
unlist(svTypical)
svTypical<-list(Linf=537.0418092,K=0.9137324,t0=1.1882746) # screwed up starting values... try spines
...
vbTypical <- tl ~ Linf*(1-exp(-K*(age-t0)))
vbTyp<-vbFuns()
vbTyp(3,Linf=c(537.0418092,0.9137324,1.1882746))
...
# subset by species then separate by lake
        #buff
buff <- subset(age, Species == "COC")
summary(buff)

ggplot(buff, aes(x=Otolith.MAS, y=Spine.MAS, shape = Lake, colour = Lake))+geom_point(size=5)+
  geom_abline(slope = 1, intercept = 0)+
  ggtitle("buff Age Estimates by Lake")
