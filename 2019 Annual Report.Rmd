...
# read in data
age<-read.csv("2017_Age_Estimates.csv",header=T)
summary(age)
str(age)


# Subset by species then lake
b.carp <- droplevels(subset(age, Species == "COC" &
                                 Lake == "Blue"))
summary(b.carp)
...
svTypical<-vbStarts(TL.mm~Otolith.MAS,data=b.carp)
...
ggplot(carp, aes(x=Otolith.MAS, y=Spine.MAS, shape = Lake, colour = Lake))+geom_point(size=5)+
