...
```{r}
datframe<-read.csv("2018 and 2017 Compiled Data.csv", header = T)
str(datframe)

# seine gear
levels(datframe$Gear)
datframe$detect<-1
seine<-droplevels(subset(datframe, Gear == "Commercial Seine"))
junk<-melt(seine, id.var = c("TAG.NUMBER","Lake"),
           measure.vars = "detect")
seine2<-dcast(junk,TAG.NUMBER~Lake)
