...
### Mean CPUE
```{r}
# will use effort cast from above, but basically merge new column of catch inststead of on time
dat.frame<-read.csv("2017-2020 Compiled Data.csv",header = T)
dat.frame$Year<-as.factor(dat.frame$Year)
dat.frame$Date<-as.Date(dat.frame$Date, "%m/%d/%Y")
dat.frame$On.Time..s.<-as.numeric(dat.frame$On.Time..s.)
...
dat.frame<-droplevels(subset(dat.frame, Species == "COC" |
                                        Species == "BIB" |
                                        Species == "dummy"))
effort.melt<-melt(dat.frame, id.var = c("Lake","Site.Transect","Date","Species"),
              measure.vars = "On.Time..s.")
effort.cast<-dcast(effort.melt, Lake + Site.Transect + Date + Species ~ ., mean,)
catch.melt<-melt(dat.frame, id.var = c("Lake","Site.Transect","Date","Species"),
              measure.vars = "detect")
catch.cast<-dcast(catch.melt, Lake + Site.Transect + Date + Species ~ ., sum)

cpue<-cbind(effort.cast,catch.cast$.)

cpue$hours<-cpue$./3600

cpue$CPUE<-cpue$`catch.cast$.`/cpue$hours
groupwiseMean(CPUE~Lake+Species,cpue)
```
...
# for AFS newsletter project update:
# length-frequencies of my 17-18 comparison lakes (center, twins)
df.b<-droplevels(subset(df.b, Lake == "Center" |
                              Lake == "N. Twin"|
                              Lake == "S. Twin"))
df.c<-droplevels(subset(df.c, Lake == "Center" |
                              Lake == "N. Twin"|
                              Lake == "S. Twin"))
...
all.lakes.Buff.lf<-ggplot(df.b.m, aes(x=value))+
                         ...
                         theme(panel.grid.major = element_blank(), 
                              ...
                              legend.background = element_rect(colour="grey20"))+
                         scale_fill_grey(start = 0.1, end = 0.9, aesthetics = "fill")+
                         labs(title = "2017 and 2018 Bigmouth Buffalo Length Frequencies",
                              x = "Length (inches)",
                              y = "Frequency")
...
head(DF2018)
DF2018<-DF2018[,-c(3,5)] # removing sample size for mean
...
levels(cpue$Lake)
cpue$Lake<-as.character(cpue$Lake)
cpue$Lake[cpue$Lake == "5 Island"]<-"Five Island"
cpue$Lake<-droplevels(as.factor(cpue$Lake))

DF2019<-groupwiseMean(CPUE~Lake+Species,cpue)
...
DF2019<-DF2019[,-c(3,5)] # removing sample size for mean
...
Now to create 4 plots, with regression lines (YAY!)
...
mod<-lm(Mean ~ BMD + Year, data = DF.17.18.19)
summary(mod)
...
CI95$Month<-as.character(CI95$Month)
CI95$Month[CI95$Month == "5"] <- "May"
CI95$Month[CI95$Month == "6"] <- "June"
CI95$Month[CI95$Month == "7"] <- "July"
CI95$Month[CI95$Month == "8"] <- "August"
CI95$Month[CI95$Month == "9"] <- "September"
CI95$Month[CI95$Month == "10"] <- "October"
CI95$Month[CI95$Month == "11"] <- "November"
CI95$Month<-as.factor(CI95$Month)
CI95$Lake<-as.character(CI95$Lake)
CI95$Lake[CI95$Lake == "5 Island"] <- "Five Island"
CI95$Lake<-as.factor(CI95$Lake)
CI95$Month<- ordered(CI95$Month, levels = c("May","June","July","August",
...
CI95.plot<-ggplot(CI95, aes(x=Month, y=Mean))+
  geom_jitter(aes(shape = Lake, colour = Lake),
              size=5, width = 0.25)+
...
# trying now with DF instead of computed means. get an idea of distribution
str(DF)
...
DF$Species[DF$Species == "BIB"] <- "Bigmouth Buffalo"
DF$Species<-as.factor(DF$Species)
DF$Month<- ordered(DF$Month, levels = c("May","June","July","August",
                                            "September","October","November"))
str(DF)

DF.boxplot<-ggplot(DF, aes(x=Lake, y=cpue))+
  ...
  labs(y = "Mean Electrofishing CPUE (Number of fish / Hour)")+
  guides(fill = guide_legend(nrow=1))+
  facet_grid(Species~Month, scales = "free_y")
DF.boxplot
...
DF2018<-groupwiseMean(cpue ~ Lake + Species + Month,
                    data = DF,
                    conf = 0.95,
                    digits = 3)
...
DF2018$L.BMD[DF2018$Lake == "Blue" &
             DF2018$Species == "COC"] <-BM$C.L.BMDensity[BM$Lake == "Blue"]
DF2018$L.BMD[DF2018$Lake == "5 Island" &
             DF2018$Species == "COC"] <-BM$C.L.BMDensity[BM$Lake == "5 Island"]
DF2018$L.BMD[DF2018$Lake == "Center" &
             DF2018$Species == "COC"] <-BM$C.L.BMDensity[BM$Lake == "Center"]
DF2018$L.BMD[DF2018$Lake == "N. Twin" &
             DF2018$Species == "COC"] <-BM$C.L.BMDensity[BM$Lake == "N. Twin"]
...
DF2018$BMD[DF2018$Lake == "Blue" &
             DF2018$Species == "COC"] <-BM$C.BMDensity[BM$Lake == "Blue"]
DF2018$BMD[DF2018$Lake == "5 Island" &
             DF2018$Species == "COC"] <-BM$C.BMDensity[BM$Lake == "5 Island"]
DF2018$BMD[DF2018$Lake == "Center" &
             DF2018$Species == "COC"] <-BM$C.BMDensity[BM$Lake == "Center"]
DF2018$BMD[DF2018$Lake == "N. Twin" &
             DF2018$Species == "COC"] <-BM$C.BMDensity[BM$Lake == "N. Twin"]
...
DF2018$U.BMD[DF2018$Lake == "Blue" &
             DF2018$Species == "COC"] <-BM$C.U.BMDensity[BM$Lake == "Blue"]
DF2018$U.BMD[DF2018$Lake == "5 Island" &
             DF2018$Species == "COC"] <-BM$C.U.BMDensity[BM$Lake == "5 Island"]
DF2018$U.BMD[DF2018$Lake == "Center" &
             DF2018$Species == "COC"] <-BM$C.U.BMDensity[BM$Lake == "Center"]
DF2018$U.BMD[DF2018$Lake == "N. Twin" &
             DF2018$Species == "COC"] <-BM$C.U.BMDensity[BM$Lake == "N. Twin"]
...
DF2018$L.BMD[DF2018$Lake == "Blue" &
             DF2018$Species == "BIB"] <-BM$B.L.BMDensity[BM$Lake == "Blue"]
DF2018$L.BMD[DF2018$Lake == "5 Island" &
             DF2018$Species == "BIB"] <-BM$B.L.BMDensity[BM$Lake == "5 Island"]
DF2018$L.BMD[DF2018$Lake == "Center" &
             DF2018$Species == "BIB"] <-BM$B.L.BMDensity[BM$Lake == "Center"]
DF2018$L.BMD[DF2018$Lake == "N. Twin" &
             DF2018$Species == "BIB"] <-BM$B.L.BMDensity[BM$Lake == "N. Twin"]
...
DF2018$BMD[DF2018$Lake == "Blue" &
             DF2018$Species == "BIB"] <-BM$B.BMDensity[BM$Lake == "Blue"]
DF2018$BMD[DF2018$Lake == "5 Island" &
             DF2018$Species == "BIB"] <-BM$B.BMDensity[BM$Lake == "5 Island"]
DF2018$BMD[DF2018$Lake == "Center" &
             DF2018$Species == "BIB"] <-BM$B.BMDensity[BM$Lake == "Center"]
DF2018$BMD[DF2018$Lake == "N. Twin" &
             DF2018$Species == "BIB"] <-BM$B.BMDensity[BM$Lake == "N. Twin"]
...
DF2018$U.BMD[DF2018$Lake == "Blue" &
             DF2018$Species == "BIB"] <-BM$B.U.BMDensity[BM$Lake == "Blue"]
DF2018$U.BMD[DF2018$Lake == "5 Island" &
             DF2018$Species == "BIB"] <-BM$B.U.BMDensity[BM$Lake == "5 Island"]
DF2018$U.BMD[DF2018$Lake == "Center" &
             DF2018$Species == "BIB"] <-BM$B.U.BMDensity[BM$Lake == "Center"]
DF2018$U.BMD[DF2018$Lake == "N. Twin" &
             DF2018$Species == "BIB"] <-BM$B.U.BMDensity[BM$Lake == "N. Twin"]
...
DF2018$Lake<-as.character(DF2018$Lake)
DF2018$Lake[DF2018$Lake == "5 Island"]<-"Five Island"
DF2018$Lake<-as.factor(DF2018$Lake)


DF2018$Month<-as.character(DF2018$Month)
DF2018$Month[DF2018$Month == "5"] <- "May"
DF2018$Month[DF2018$Month == "6"] <- "June"
DF2018$Month[DF2018$Month == "7"] <- "July"
DF2018$Month[DF2018$Month == "8"] <- "August"
DF2018$Month[DF2018$Month == "9"] <- "September"
DF2018$Month[DF2018$Month == "10"] <- "October"
DF2018$Month[DF2018$Month == "11"] <- "November"
DF2018$Month<-as.factor(DF2018$Month)
DF2018$Lake<-as.character(DF2018$Lake)
DF2018$Lake[DF2018$Lake == "5 Island"] <- "Five Island"
DF2018$Lake<-as.factor(DF2018$Lake)
DF2018$Month<- ordered(DF2018$Month, levels = c("May","June","July","August",
                                            "September","October","November"))
...
levels(DF2018$Species)
DF2018$Species<-as.character((DF2018$Species))
DF2018$Species[DF2018$Species == "BIB"]<-"Bigmouth Buffalo"
DF2018$Species[DF2018$Species == "COC"]<-"Common Carp"
DF2018$Species<-as.factor(DF2018$Species)




BMD.plot<-ggplot(DF2018, aes(x= BMD,y=Mean))+
...
DF2018.b<-droplevels(subset(DF2018, Species == "Bigmouth Buffalo"))

BMD.plot.b<-ggplot(DF2018.b, aes(x= BMD,y=Mean))+
                  geom_point(aes(shape = Lake),size=2)+
