...
# here's a hack at it:::::
    # gonna subset all dummy to a new df, set those species names to bib
dummy.df<-droplevels(subset(catch.df, Species == "dummy"))
dummy.df$`1`[dummy.df$Species == "dummy"]<-0
dummy.df<-droplevels(subset(dummy.df, Lake != "Blue")) # no blue buffalo
dummy.df$Species<-as.character(dummy.df$Species)
dummy.df$Species[dummy.df$Species == "dummy"]<-"BIB"
    # then take catch.df and set those dummy species to coc
catch.df$`1`[catch.df$Species == "dummy"] <- 0
catch.df$Species<-as.character(catch.df$Species)
catch.df$Species[catch.df$Species == "dummy"]<-"COC"
    # then rbind the bib dataset to catch.df
catch.df<-rbind(catch.df,dummy.df)

# potential problem with the cast function!!



catch.df$CPUE<-catch.df$`1`/(catch.df$On.Time..s./3600)

# clean up some labels
catch.df$Species[catch.df$Species == "COC"] <-"Common Carp"
catch.df$Species[catch.df$Species == "BIB"] <-"Bigmouth Buffalo"
catch.df$Species<-droplevels(as.factor(catch.df$Species))
#
...
DF.boxplot<-ggplot(catch.df, aes(x=Month, y=CPUE))+
  ...
  facet_grid(Species~Lake, scales = "free")
DF.boxplot
```
...
cpue$Month<-factor(format(cpue$Date, "%m"))
DF2019<-groupwiseMean(CPUE~Lake+Month+Species,cpue)
warnings()
DF2019<-droplevels(subset(DF2019, Species == "COC" |
                                  Species == "BIB"))
...
str(DF2019) # # have to figure out how to do this appropriately. Why do I have uneven carp/buff runs....
...
DF2019$Month[DF2019$Month == "5"] <- "May"
DF2019$Month[DF2019$Month == "6"] <- "June"
DF2019$Month[DF2019$Month == "7"] <- "July"
DF2019$Month[DF2019$Month == "8"] <- "August"
DF2019$Month[DF2019$Month == "9"] <- "September"
...
DF2019$Lake[DF2019$Lake == "5 Island"] <- "Five Island"
DF2019$Lake<-as.factor(DF2019$Lake)
DF2019$Month<- ordered(DF2019$Month, levels = c("May","June","July","August",
